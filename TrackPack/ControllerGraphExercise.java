...
public class ControllerGraphExercise implements Initializable {
    @FXML
    MenuBar menuBar;
    @FXML
    LineChart<Number, Number> timeline;
    @FXML
    ChoiceBox<String> exerciseChoice;
    @FXML
    ComboBox<String> routineChoice;
    ...
    public void routineSelected(){
        ...
        int i = 0;
        while (!(tempRoutineRecord.get(i).getWorkout().equals(routineChoice.getValue()))){
            i++;
        }
        String[] exercises = tempRoutineRecord.get(i).getExercises();
        for (int j = 0; j < exercises.length; j++){
            exerciseChoice.getItems().add(exercises[j]);
        }
    }
    public void addBClicked(){
        String exercise = exerciseChoice.getValue();
        ...
        int pos = 0;
        int prevI = 0;
        while(!(exercise.equals(tempRec.get(prevI).getExercise()))){
            prevI++;
        }
        for (int i = prevI+1; i < tempRec.size(); i++){
            if (exercise.equals(tempRec.get(i).getExercise())) {
